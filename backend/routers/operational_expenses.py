...
def create_operational_expense(expense: schemas.OperationalExpenseCreate, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
...
def read_operational_expenses(start_date: date, end_date: date, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
...
def update_operational_expense(expense_id: int, expense: schemas.OperationalExpenseUpdate, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
...
def delete_operational_expense(expense_id: int, db: Session = Depends(get_db), tenant_id: int = Depends(get_tenant_id)):
