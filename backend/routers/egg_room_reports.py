...
@router.post("/initial-setup", response_model=EggRoomReportResponse)
def create_or_update_initial_report(report: EggRoomReportCreate, db: Session = Depends(get_db)):
    """
    Create or update an egg room report for initial setup, bypassing system_start_date validation.
    If a report for the given date exists, it will be updated. Otherwise, it will be created.
    """
    try:
        requested_date = report.report_date  # Already a date object from Pydantic
        # No system_start_date validation here
        existing_report = egg_crud.get_report_by_date(db, requested_date.isoformat())
        if existing_report:
            # Update the existing report
            updated_report = egg_crud.update_report(db, requested_date.isoformat(), report)
            if updated_report:
                result = {
                    "report_date": updated_report.report_date.isoformat(),
                    "table_received": updated_report.table_received,
        ...
        logger.error(f"Error in initial setup report: {e}\n{traceback.format_exc()}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error")