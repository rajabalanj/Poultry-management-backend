...
router = APIRouter(
    ...
    dependencies=[Depends(require_group(["super_admin"]))] # Only Super Admins can access!
)

@router.get("", response_model=List[TenantFeature])
...
def read_features_for_tenant(tenant_id: str, db: Session = Depends(get_db)):
    return crud_tenant_feature.get_features_by_tenant(db=db, tenant_id=tenant_id)

@router.post("", response_model=TenantFeature, status_code=status.HTTP_201_CREATED)
...
@router.patch("/{feature_id}", response_model=TenantFeature)
