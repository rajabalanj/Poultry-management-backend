...
def create_composition(db: Session, composition: CompositionCreate, tenant_id: str):
    db_composition = Composition(name=composition.name, tenant_id=tenant_id)
...
def update_composition(db: Session, composition_id: int, composition: CompositionCreate, tenant_id: str):
...
def delete_composition(db: Session, composition_id: int, tenant_id: str):
    db_composition = db.query(Composition).filter(Composition.id == composition_id, Composition.tenant_id == tenant_id).first()
    if not db_composition:
        return False
